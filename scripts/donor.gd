...
func _physics_process(delta):
	...
	if tazertarget.size() <= 0:
		...
		for i in tazertarget.size():
			#cool lazer code
			$shock.look_at(tazertarget[i].get_global_position())
			$shock.on = true
			#$shock.col_point = tazertarget[i].get_global_position()#doesnt work
			$shock.distance = get_global_position().distance_to(tazertarget[i].get_global_position())
			#$shock.col_point =tazertarget[i].get_position()-get_global_position()
			#draw_line(Vector2.ZERO,tazertarget[i].get_position()-get_global_position(),Color.darkgoldenrod,3.0)
			tazertarget[i].shock()
